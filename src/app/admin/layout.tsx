  ...
  return (
    <div className="flex justify-end">
      ...
      >
        ...
        <button
          className={`transition-all w-6 h-1/5 bg-[#181818] rounded-tr-full fixed rounded-br-full  bottom-1/2 ${isVisible ? "  self-center transition-transform duration-500 ease-in-out" : " transform  transition-transform duration-500 ease-in-out"}`}
          onClick={() => {
            setIsVisible(!isVisible)
          }}
        >
      ...
      <div
        className={`${isVisible ? " w-[80vw] transform-origin: right duration-500 ease-in-out " : "w-full transform-origin: left duration-500 ease-in-out "}`}>
        <section className="w-full h-full">
          <div className="p-4 pl-8 ">
            <HeaderAdmin />
            <div
              className="w-full max-h-[80vh] p-4 shadow rounded-lg flex items-center justify-center overflow-auto">
              {children}
            </div>
          </div>
          <Footer
            width={`${isVisible ? "w-[80vw] transition-all" : "w-full transition-all"}`}></Footer>
        </section>
      </div>
    </div>
  )
